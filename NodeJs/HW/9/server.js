...
app.get('/students/edit', (req, res) => 
{
    res.send(`
        ...
        <form action="/students/edit" method="post">
            ...
            <input type="text" id="name" name="name" required>
            <label for="groupId">New Group ID:</label>
            <input type="number" id="groupId" name="groupId" required>
...
app.post('/students/edit', (req, res) => 
{
    ...
    mssql.connect(config).then(pool =>
    {
        return pool.request()
            ...
            .input('newName', mssql.VarChar, newName)
            .input('newGroupId', mssql.Int, newGroupId)
            .query('UPDATE Students SET Name = @newName, GroupID = @newGroupId WHERE StudentID = @studentId');
    }).then(result => 
    {
        res.send(`
            <h1>Student with ID ${studentId} updated to "${newName}" in Group ${newGroupId} successfully</h1>
            <a href="/students">Back to Students</a>
        `);
    }).catch(err => 
    {
